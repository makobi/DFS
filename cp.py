...
if command == "-t": # To DFS:
	...
	for i in range (0, len(data)-1): # For each available node:
		...
		blockInfo = "0//" + str(i) + "//" + chunkNames[i] + "//" + str(fname[0]) # Isolate the chunk.
	...
	data = s.recv(640000000)        # Receive info of where the file is stored, i.e. Data-nodes.
	...
	while not (i >= (len(data)-1)): # For each data-node:
		...
		blockInfo = "1//" + res[3] + "//" + str(fname[0]) # Isolate the chunk.
